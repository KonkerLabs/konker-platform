...
def save_incoming_events(incomingEvents, host):
    ...
    for event in incomingEvents:
        try:
            db.execute_async(
                """
                INSERT INTO incoming_events (device_guid, timestamp, channel, device_id, payload, tenant_domain, application_name, deleted)
                ...
                """, (event['incoming']['deviceGuid'],
                      ...
                      event['incoming']['applicationName'],
...
def save_outgoing_events(outgoingEvents, host):
    ...
    for event in outgoingEvents:
        try:
            db.execute_async(
                """
                INSERT INTO outgoing_events (device_guid, timestamp, channel, device_id, payload, tenant_domain, application_name, deleted, incoming)
...
def find_incomingEvents_by_timestamp(timestamp, tenant, host):
    ...
    try:
        if tenant is None:
            query = """
                SELECT device_guid, timestamp, channel, device_id, payload, tenant_domain, application_name, deleted FROM 
...
def find_outgoingEvents_by_timestamp(timestamp, tenant, host):
    ...
    try:
        if tenant is None:
            query = """
                SELECT device_guid, timestamp, channel, device_id, payload, tenant_domain, application_name, deleted, incoming FROM 
