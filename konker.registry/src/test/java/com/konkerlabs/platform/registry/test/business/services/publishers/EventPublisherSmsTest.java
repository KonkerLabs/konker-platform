...
import com.konkerlabs.platform.registry.business.services.routes.publishers.EventPublisherSms;
...
public class EventPublisherSmsTest extends BusinessLayerTestSupport {
    ...
    public void shouldSendEventThroughGateway() throws Exception {
        ...
        Mockito.verify(eventRepository).push(tenant,event);
        Mockito.verify(smsMessageGateway).send(eq(expectedMessage),eq(smsPhoneNumber));
    }

    @Test
    public void shouldNotSendEventThroughGatewayIfItCouldNotBeLogged() throws Exception {
        doThrow(BusinessException.class).when(eventRepository).push(tenant,event);

        subject.send(event,destinationUri,null,tenant);

        Mockito.verify(smsMessageGateway,never()).send(anyString(),anyString());
    }
}