    ...
    protected void append(Object eventObject) {
        ...
        if (eventObject != null && eventObject instanceof LoggingEvent) {
            ...
            if (((LoggingEvent) eventObject).getArgumentArray() != null
                ...
                Arrays.stream(((LoggingEvent) eventObject).getArgumentArray()).forEach(arg -> {
                    if (arg instanceof URI && !((URI) arg).getScheme().equals("http")) {
                        MDC.put(CONTEXT, encodeDealer((URI) arg));
                    }
                });
            }
